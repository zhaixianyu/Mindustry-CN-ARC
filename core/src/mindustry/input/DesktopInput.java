...
public class DesktopInput extends InputHandler{
    ...
    public void drawTop(){
        ...
        if(mode == breaking){
            drawBreakSelection(selectX, selectY, cursorX, cursorY, !Core.input.keyDown(Binding.schematicSelect) ? maxLength : ARCVars.getMaxSchematicSize());
        }
    ...
    public void update(){
        ...
        if(!scene.hasField() && !scene.hasDialog()){
            ...
            if((Math.abs(Core.input.axis(Binding.moveX)) > 0 || Math.abs(Core.input.axis(Binding.moveY)) > 0 || input.keyDown(Binding.mouseMove)) && (!scene.hasField())){
    ...
    void pollInput(){
        ...
        if(Core.input.keyRelease(Binding.breakBlock) || Core.input.keyRelease(Binding.select)){
            ...
            if(mode == placing && block != null){ //touch up while placing, place everything in selection
                ...
                removeSelection(selectX, selectY, cursorX, cursorY, !Core.input.keyDown(Binding.schematicSelect) ? maxLength : ARCVars.getMaxSchematicSize());
