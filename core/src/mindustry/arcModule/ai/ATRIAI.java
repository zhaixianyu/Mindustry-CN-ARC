    ...
    private void builderMode(){
        ...
        if(unit.buildPlan() != null){
            ...
            if(!unit.team.data().plans.isEmpty() && following == null && timer.get(timerTarget3, rebuildTime)){
                ...
                if(world.tile(block.x, block.y) != null && world.tile(block.x, block.y).block().id == block.block.id){
                    blocks.removeFirst();
                }else if(Build.validPlace(block.block, unit.team(), block.x, block.y, block.rotation) && (!nearEnemy(block.x, block.y))){ //it's valid
                    lastPlan = block;
                    //add build plan
